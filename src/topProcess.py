...
class topProcess:
	...
	def startTop(self):
		...
		try:
		 ...
		 # m.sendline('cd /mnt/10.211.8.160')
		 # m.expect([pexpect.EOF,'#'])
		 # m.sendline('mkdir top')
		 # m.expect([pexpect.EOF,'#'])
		 # m.sendline('cd top')
		 # m.expect([pexpect.EOF,'#'])
		  m.sendline('while true; do top -Mbn 1; cat /proc/meminfo; free -m; slabtop -s c -o; ps auxk rss | grep -v \' 0.0  0.0      0     0\'; sleep 15; done |tee top.txt')
		  print "hello"
